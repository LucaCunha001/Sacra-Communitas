...
async def log_punicao(
    ...
    view = PunicaoLayout(titulo, descricao, cor, membro, author, motivo, extras)

    if canal_punicoes:
        await canal_punicoes.send(view=view)
    else:
        logger.warning(
            "Canal de punições não encontrado: %s",
            get_config()["canais"].get("punicoes"),
        )
...
async def log_normal(
    ...
    if canal:
        ...
        logger.warning("Canal de log não encontrado: %s", canal_id)
