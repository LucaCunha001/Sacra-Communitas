...
class Bot(commands.Bot):
    ...
    async def load_cogs(self):
        for extension in os.listdir("cogs"):
            if extension.endswith(".py"):
                module_name = f"cogs.{extension[:-3]}"
                if module_name == "cogs.__init__":
                    continue
                try:
                    ...
                    logger.info("Cog recarregado: %s", module_name)
                except Exception:
                    logger.exception("Falha ao carregar cog %s", module_name)
    ...
    async def send_to_console(
        ...
        *,
        ...
        logger.warning("Canal de console não encontrado: 1462077223140851903")
