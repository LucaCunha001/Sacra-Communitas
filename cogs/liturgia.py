...
class LiturgiaCog(commands.Cog):
	...
	async def calendario(self, interaction: discord.Interaction, ano: int | None = None) -> None:
		cores = {
			"Quaresma": 0x6A0DAD,
		...
		def primeiro_domingo_do_ano(ano: int) -> datetime.date:
	...
	async def hora_config(self, interaction: discord.Interaction, hora: str) -> discord.InteractionCallbackResponse[discord.Client] | None:
	...
	@permissao(gerenciar_comunidade=True)
	async def canal_config(self, interaction: discord.Interaction, webhook_url: str) -> None:
	...
	@permissao(gerenciar_comunidade=True)
	async def ping_config(self, interaction: discord.Interaction, ping: discord.Role) -> None:
	...
	async def envio_liturgia(self) -> None:
		...
		if not isinstance(canal, discord.TextChannel):
			await self.bot.send_to_console(f"Canal {config['canal']} não é um canal de texto.")
			return
		...
		try:
			...
			traceback.print_exc()
			await self.bot.send_to_console(f"Erro ao enviar liturgia: {e}")

	@envio_liturgia.before_loop
	async def before_envio(self) -> None:
	...
	async def generate_liturgy_view(self, url: str, content: str) -> list[ui.LayoutView]:
		...
		def get_container_color(cor: str) -> int:
			cores = {
				"Verde": 0x00FF00,
				"Branco": 0xFFFFFF,
				"Vermelho": 0xFF0000,
