    ...
    def __init__(self, bot: Bot):
        ...
        for i, opcao in enumerate(self.opcoes):
            self.add_option(
                ...
                emoji=opcao[0],
                value=str(i),
                description=opcao[2] if len(opcao) == 3 else None,
            )
...
async def create_ticket_channel(
    bot: Bot, interaction: discord.Interaction, channel_name: str
):
    config = bot.config
...
class TicketView(discord.ui.View):
    ...
    async def fechar_ticket(self, channel: discord.TextChannel, staff: discord.User):
        ...
        embed = self._create_ticket_embed(channel, ticket_owner, staff)
        mensagens = await self._count_messages(channel)
        transcript_html = await self._generate_transcript(channel, mensagens)
        await self._send_transcripts(ticket_owner, embed, transcript_html, channel)
