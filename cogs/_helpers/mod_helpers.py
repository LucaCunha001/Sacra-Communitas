...
from utils.data import WarnsJson, get_member, save_member
from utils.logs import make_embed, log_punicao, TipoPunicao
...
class RemoveWarnOptions(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if index >= len(membro_json["warns"]):
            return await interaction.response.send_message(
                "⚠️ Essa penitência já foi removida.", ephemeral=True
            )
...
class WarnGP(app_commands.Group):
    ...
    async def warn_add(
        ...
        novo_warn: WarnsJson = {
            ...
            "motivo": motivo,
            "quando": datetime.datetime.now(datetime.timezone.utc).timestamp(),
            "remocao": False,
        }
        ...
        try:
            if interaction.guild.me.top_role > membro.top_role:
                ...
                await interaction.followup.send(
                    "⚠️ Warn aplicado, mas não consegui aplicar timeout por hierarquia de cargos.",
                    ephemeral=True,
                )
        except discord.Forbidden:
            await interaction.followup.send(
                "⚠️ Warn aplicado, mas não tenho permissão para aplicar timeout.",
                ephemeral=True,
            )
        ...
        await log_punicao(
            interaction.guild, TipoPunicao.Warn, membro, interaction.user, motivo
        )
    ...
    async def warn_logs(self, interaction: discord.Interaction, membro: discord.Member):
        ...
        if not membro_json["warns"]:
            return await interaction.response.send_message(
                "Esse membro não possui penitências.", ephemeral=True
            )
    ...
    @app_commands.command(
        name="remove", description="Remove uma penitência de um membro"
    )
    @app_commands.describe(
        membro="Membro que terá a penitência removida", motivo="Motivo da remoção"
    )
    @is_staff()
