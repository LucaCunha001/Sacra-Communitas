...
async def buscar_noticia_por_guid(
	...
	try:
		...
		for item in root.findall("channel/item"):
			...
			guid = (
				...
				else (link_elem.text.strip() if link_elem is not None else "")
			)
...
class VaticanNewsCog(commands.Cog):
	...
	@news_gp.command(
		name="force", description="Força o bot a enviar uma notícia específica."
	)
	async def force_news(self, interaction: discord.Interaction, guid: str):
		if not await self.bot.is_owner(interaction.user):
			return await interaction.response.send_message("Você não tem permissão de usar esse comando.")
		...
		noticia = await buscar_noticia_por_guid(self.session, guid)
		if not noticia:
			return
		...
		await interaction.response.send_message("Notícia enviada com sucesso!", ephemeral=True)
