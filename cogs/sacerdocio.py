...
from ._helpers.ticket_views import create_ticket_channel
...
class RecusarSolicitacaoModal(ui.Modal, title="Recusar Solicitação de Sacerdócio"):
    ...
    def __init__(
        self, bot: Bot, usuario: discord.Member, disable_all: Callable[..., None]
    ):
        super().__init__()
        self.bot = bot
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await self.usuario.remove_roles(
            interaction.guild.get_role(
                self.bot.config["cargos"]["membros"]["Noviço"]["id"]
            )
        )
...
class SolicitacaoSacerdocioView(ui.View):
    ...
    @ui.button(
        ...
        custom_id="recusar_solicitacao_sacerdocio",
        emoji="❌",
    )
    async def recusar_solicitacao_button(
        self, interaction: discord.Interaction, button: ui.Button
    ):
        usuario = await self.extract_info(interaction)
        if usuario is not None:
            if any(
                interaction.guild.get_role(r.get("id")) in usuario.roles
                for r in self.bot.config["cargos"]["sacerdotes"].values()
            ):
            ...
            if not any(
                r.id == self.bot.config["cargos"]["membros"]["Noviço"]["id"]
            ...
            return await interaction.response.send_modal(
                RecusarSolicitacaoModal(
                    bot=self.bot, usuario=usuario, disable_all=func_da
                )
            )
...
class SolicitacaoSacerdocioModal(ui.Modal, title="Solicitação de Sacerdócio"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed.add_field(
            name="Usuário", value=f"{user.mention} (`{user.id}`)", inline=False
        )
        ...
        embed.add_field(
            name="Experiência:", value=self.experiencia or "Não informada", inline=True
        )
        embed.add_field(name="Sobre o candidato:", value=self.pouco_sobre, inline=False)
        embed.set_footer(
            text=f"Solicitação enviada por {user.display_name}",
...
async def enviar_solicitacao(bot: Bot, interaction: discord.Interaction):
    if not any(
        ...
        for r in [1429972827209207878, 1429973276041412658]
    ):
        return await interaction.response.send_message(
            "Precisa ser católico para se tornar um sacerdote!", ephemeral=True
        )
    ...
    if any(
        r.id == bot.config["cargos"]["membros"]["Noviço"]["id"]
        for r in interaction.user.roles
    ):
        await interaction.response.send_message(
            "Você já fez a sua solicitação e não pode enviar outra.", ephemeral=True
        )
        return

    if any(
        cargo_dict["id"] in [role.id for role in interaction.user.roles]
        for cargo_dict in cargos_sacedotes.values()
    ):
        await interaction.response.send_message(
            "Você já é um sacerdote e não pode enviar outra solicitação.",
            ephemeral=True,
        )
        return
    ...
    async for msg in canal.history(limit=None, oldest_first=False):
        ...
        if embed.fields and embed.fields[0].value.startswith(interaction.user.mention):
            if msg.created_at < datetime.datetime.now() - datetime.timedelta(days=7):
                return await interaction.response.send_message(
                    f"Você já enviou uma solicitação há menos de uma semana e não pode enviar outra { discord.utils.format_dt(msg.created_at + datetime.timedelta(days=7), "R") }.",
                    ephemeral=True,
                )
...
class InscricoesView(ui.LayoutView):
    ...
    async def enviar(self, interaction: discord.Interaction):
        if not any(
            r.id in [1429972827209207878, 1429973276041412658]
            for r in interaction.user.roles
        ):
            return await interaction.response.send_message(
                "Precisa ser católico para se inscrever!", ephemeral=True
            )
        ...
        if tipo == "novico":
            ...
            if any(r.id == 1461744600061575366 for r in interaction.user.roles):
                return await interaction.response.send_message(
                    "Você já faz parte da secretaria do servidor.", ephemeral=True
                )

            await create_ticket_channel(
                self.bot, interaction, f"📜・{interaction.user.name}"
            )
        elif tipo == "teologia":
            if any(r.id == 1468027291199078495 for r in interaction.user.roles):
                return await interaction.response.send_message(
                    "Você já faz parte da equipe teológica do servidor.", ephemeral=True
                )
...
class SacerdocioCog(commands.Cog):
    ...
    sacerdocio_group = app_commands.Group(
        name="sacerdocio", description="Comandos relacionados aos sacerdócios"
    )

    @sacerdocio_group.command(
        name="info", description="Obter informações sobre o sacedócio."
    )
    async def info(self, interaction: discord.Interaction):
        ...
        embed.set_thumbnail(
            url="https://upload.wikimedia.org/wikipedia/commons/thumb/8/81/Emblem_of_the_Papacy_SE.svg/250px-Emblem_of_the_Papacy_SE.svg.png"
        )
    ...
    @sacerdocio_group.command(
        name="noviços", description="Mostra os noviços do servidor."
    )
    @permissao(designar_cargos=True)
    async def novicos(self, interaction: discord.Interaction):
        novico_cargo = interaction.guild.get_role(
            self.bot.config["cargos"]["membros"]["Noviço"]["id"]
        )
        ...
        if not novicos:
            return await interaction.response.send_message(
                "Não há noviços no momento.", ephemeral=True
            )
    ...
    @sacerdocio_group.command(
        name="solicitacoes_msg",
        description="Envia as mensagens de solicitações no canal designado.",
    )
    ...
    async def solicitacao_msg(self, interaction: discord.Interaction):
        ...
        if not canal:
            return await interaction.response.send_message(
                "Canal de solicitações de sacerdócio não encontrado.", ephemeral=True
            )
        ...
        await interaction.response.send_message(
            "Mensagem de solicitação enviada com sucesso!", ephemeral=True
        )

    @sacerdocio_group.command(
        name="enviar_solicitacao",
        description="Enviar uma solicitação para se tornar um sacerdote.",
    )
    ...
    async def anjos(self, interaction: discord.Interaction):
        ...
        for nome, cargo_dict in list(anjos_config.items())[::-1]:
            triade_idx = next(
                (idx for idx, triade in enumerate(ordens) if nome in triade), None
            )
            ...
            embed.description += (
                f"{cargo.mention} - {cargo_dict.get('descricao', '')}\n"
            )
    ...
    async def update_sacerdote(
        ...
        if sacerdote.bot and responder_interacao:
            return await interaction.response.send_message(
                "Eu não sei o que te fez pensar que poderia mexer nas permissões de um ser angelical.",
                ephemeral=True,
            )
        ...
        if tipo == 2:
            ...
            if funcao and funcao >= cargo_maximo_atual:
                if responder_interacao:
                    await interaction.response.send_message(
                        "O novo cargo deve ser **inferior** ao atual.", ephemeral=True
                    )
                return
        ...
        citacoes = [
            # Ingressão
            ["1Tm 3,13", "Ap 21,5", "1Sm 16,7", "Sb 6,1-3", "Cl 3,23", "Pr 22,29"],
            # Promoção
            ["Lc 12:48", "1Pd 5,2-3", "Mt 25:21", "Sb 6,20-21", "Rm 12,8", "2Tm 2,15"],
            # Rebaixamento
            ["Hb 12,6", "Sl 51,10", "Pr 3,11-12", "Lm 3,31-33", "Mq 6,8", "Sl 119,71"],
            # Remoção
            ["Pr 24,16", "2Cor 12,9", "Jó 1,21", "Ecl 3,1", "Sl 34,19", "Is 55,8-9"],
        ]
        ...
        passagem_txt = (
            res["versículo_inicial"]
            if res["versículo_final"] == res["versículo_inicial"]
            else f"{res['versículo_inicial']}-{res['versículo_final']}"
        )
        ...
        match tipo:
            ...
            case 1:
                ...
                if (
                    no_servidor
                    and cargo_maximo_atual
                    and cargo_maximo_atual < bispo_role
                ):
            ...
            case 2:
                ...
                if no_servidor and cargo_maximo_atual:
                    await sacerdote.remove_roles(
                        cargo_maximo_atual, reason="Rebaixamento"
                    )
            ...
            case 3:
                ...
                if no_servidor:
                    for _, cargo_dict in self.bot.config["cargos"][
                        "sacerdotes"
                    ].items():
        ...
        if responder_interacao:
            await interaction.response.send_message(
                "Cargo alterado com sucesso!", ephemeral=True
            )
        ...
        container.add_item(ui.TextDisplay("-# Sacra Communitas • Conclave"))
        view.add_item(container)
    ...
    @sacerdocio_group.command(
        name="nomear", description="Nomeia um noviço à função sacerdotal."
    )
    @permissao(designar_cargos=True)
    @app_commands.describe(sacerdote="O membro que será nomeado noviço")
    async def nomear(self, interaction: discord.Interaction, sacerdote: discord.Member):
        cargo = interaction.guild.get_role(
            int(self.bot.config["cargos"]["sacerdotes"]["Seminarista"]["id"])
        )
        await self.update_sacerdote(
    ...
    @sacerdocio_group.command(
        name="ordenar", description="Ordena um sacerdote a um cargo maior."
    )
    ...
    async def ordenar(
        self, interaction: discord.Interaction, sacerdote: discord.Member, cargo: str
    ):
    ...
    @sacerdocio_group.command(
        name="exonerar", description="Rebaixa um sacerdote a um cargo menor."
    )
    @app_commands.choices(
        cargo=[
            app_commands.Choice(name=cargo, value=str(cargo_dict["id"]))
    ...
    @sacerdocio_group.command(
        name="dispensar", description="Dispensa um sacerdote de suas funções."
    )
    @permissao(designar_cargos=True)
    @app_commands.describe(
...
class ModalRegras(discord.ui.Modal):
    def __init__(self, i: int):
        super().__init__(
            title="Mudar regra do servidor", timeout=None, custom_id="regras_servidor"
        )
        ...
        self.add_item(
            discord.ui.TextInput(
                label=f"Regra {contar(i+1)}",
                style=discord.TextStyle.long,
                custom_id=str(i),
                id=i,
                default=self.regras[i] if len(self.regras) > i else None,
                required=True,
                placeholder=f"Regra de número {i+1}",
            )
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            "Regras atualizadas com sucesso!", ephemeral=True
        )
...
class CanonesCog(commands.Cog):
    ...
    regras_gp = app_commands.Group(
        name="regras", description="Comandos relacionados as regras do servidor."
    )
    ...
    async def regras_update(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            "Regras enviadas com sucesso!", ephemeral=True
        )

    def gerar_regras_embed(self, guild: discord.Guild) -> discord.Embed:
        regras = self.get_regras()
        embed = discord.Embed(
            title=f"Decálogo da {guild.name}", description="", color=0xFFCC00
        )

        embed.set_footer(text=f"{guild.name} - Decálogo", icon_url=guild.icon.url)
        embed.set_thumbnail(
            url="https://upload.wikimedia.org/wikipedia/commons/thumb/8/81/Emblem_of_the_Papacy_SE.svg/250px-Emblem_of_the_Papacy_SE.svg.png"
        )
        embed.set_author(name=guild.owner.name, icon_url=guild.owner.display_avatar.url)
    ...
    @app_commands.describe(numero="A posição com a qual a regra situará.")
    @app_commands.checks.has_permissions(administrator=True)
    ...
    canone_gp = app_commands.Group(
        name="canone", description="Comandos relacionados aos cânones da comunidade."
    )

    @canone_gp.command(
        name="setup", description="Envia os cânones dos seus respecitvos canais."
    )
    @app_commands.checks.has_permissions(administrator=True)
    async def setup_canone(self, interaction: discord.Interaction):
        ...
        embed_codigo.set_author(
            name=interaction.user.name,
            url=f"https://discord.com/users/{interaction.user.id}",
            icon_url=interaction.user.display_avatar.url,
        )
        embed_codigo.set_footer(
            text=f"{interaction.guild.name} - Código Canônico",
        ...
        for c, canone in enumerate(canones):
            ...
            for artigo in canone["artigos"]:
                ...
                contagem_artigos += 1
                embed.description += (
                    f"**Art. {contar(contagem_artigos)}º** {artigo['texto']}\n"
                )
                ...
                embed.description += (
                    "\n"
                    if not any(
                        len(artigo.get(j, [])) != 0 for j in ["incisos", "paragrafos"]
                    )
                    else "\n\n"
                )
